...
const generateItems = (count: number): CarouselItem[] => {
  return Array.from({ length: count }).map((_, i) => {
    const isRectangle = i % 12 === 0; // Keep only a handful of rectangles.
    const isLandscapeRectangle = Math.floor(i / 12) % 2 === 0;

    // Squares are intentionally larger than rectangles.
    let width = 2.25;
    let height = 2.25;

    if (isRectangle) {
      if (isLandscapeRectangle) {
        width = 1.95;
        height = 1.3;
      } else {
        width = 1.3;
    ...
    // High resolution for clean textures.
    const wPx = Math.floor(width * 320);
    const hPx = Math.floor(height * 320);
...
const INITIAL_ITEMS = generateItems(90); // Slightly reduced density

const Loader = () => (
  <Html center>
    <div className="flex flex-col items-center gap-4">
      <div className="w-12 h-12 border-4 border-[#5E3A28] border-t-transparent rounded-full animate-spin"></div>
      <div className="text-[#5E3A28] font-bold text-xs tracking-[0.2em] uppercase">
        Loading Experience
      </div>
    </div>
  </Html>
...
const App: React.FC = () => {
  ...
  return (
    <div className="w-full h-screen relative overflow-hidden bg-[#F5E6D8]">
      ...
      <div
        className={`absolute inset-0 z-0 transition-all duration-700 ease-out ${lightbox.isOpen ? "blur-md scale-95 opacity-50 grayscale" : ""}`}
      >
        <Canvas dpr={[1, 2]} shadows>
          <Suspense fallback={<Loader />}>
            <GlobeScene items={items} onItemClick={handleItemClick} />
          </Suspense>
        </Canvas>
      </div>
